    ...
    <header className="font-dm-sans fixed h-16 w-full bg-[var(--primary-bg)] transition-color duration-150 z-10 flex flex-row justify-between items-center px-4 lg:px-8 border-b-[1px] border-b-[var(--outline)]">
      ...
      <div className="w-48 h-full hidden lg:flex flex-row justify-center">
        <Link className="outline-none h-full" to={"/"}>
          <button
            className={`text-[var(--text)] transition-color duration-150 h-full w-36 font-semibold hover:bg-[var(--badge-btn-hover)] ${
        ...
        <Link className="outline-none h-full" to={"/Balanced"}>
          <button
            ...
            //   setDiet(["Balanced"]);
            // }}
            className={`text-[var(--text)] transition-color duration-150 h-full w-32 font-semibold hover:bg-[var(--badge-btn-hover)] ${
      ...
      <div className="w-40 lg:w-56 flex flex-row justify-end gap-0 lg:gap-3 items-center">
        <div
          onClick={() => {
        ...
        >
          {darkMode && (
            <FaRegSun
              ...
              className="w-6 h-auto cursor-pointer transition-color duration-150"
            />
          )}
          {!darkMode && (
            <FaRegMoon
              style={{ fill: darkMode ? "#ffffff" : "#000000" }}
              className="w-6 h-auto py-1 cursor-pointer"
            />
          )}
        </div>
        <Link to={"/savedRecipes"}>
          <button className="text-sm bg-[var(--orange)] hover:bg-[var(--orange-hover)] transition-color duration-150 px-4 py-[10px] whitespace-nowrap rounded-md text-white font-semibold hidden lg:flex flex-row gap-2 items-center justify-center">
            <LuBookMarked className="h-4 w-auto" /> <span>Saved Recipes</span>
          </button>
        </Link>
      </div>
    </header>
