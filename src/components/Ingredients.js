    ...
    <div className="text-[var(--text)] lg:text-sm text-base flex flex-col">
      ...
      >
        <div className="flex flex-row items-center gap-4">
          <GiPressureCooker className="h-7 lg:h-6 2xl:h-9 w-auto transition-colors duration-150" />
          <span className="transition-colors duration-150 2xl:text-xl">Ingredients</span>
          {ingredients.length ? (
          ...
          {optionArr.map((option) => {
            return (
              <label
                ...
                } cursor-pointer px-2 2xl:text-lg py-1 border-[1px] border-[var(--outline)] rounded-md`}
                htmlFor={option}
                key={option}
              >
                <input
